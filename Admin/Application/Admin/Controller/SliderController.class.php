...
class SliderController extends AdminController {
	...
	public function add() {
		if (IS_POST) {
			$Slider = D ( 'Slider' );
			$data = $Slider->create ();
            $Slider->create_time = time();
			//$this->error (json_encode($data));
			if ($data) {
				$id = $Slider->add ();
				if ($id) {
					// 记录行为
					action_log ( 'add_Slider', 'Slider', $id, UID );
...
//			$this->assign ( 'category', $category );
	...
	public function edit($id = 0) {
		if (IS_POST) {
			...
			if ($data) {
				if ($Slider->save ()) {
					// 记录行为
					action_log ( 'edit_Slider', 'Slider', $data ['id'], UID );
			...
			$info = array ();
			/* 获取数据 */
			$info = M ( 'Slider' )->field ( true )->find ( $id );
	...
	public function delete() {
		...
		if (M ( 'Slider' )->where ( $map )->delete ()) {
			// 记录行为
			action_log ( 'delete_Slider', 'Slider', $id, UID );
			$this->success ( '删除成功' );
		} else {
			$this->error ( '删除失败！' );
		}
