...
<li><a href="javascript:history.back(-1)">返回上一页</a> <span class="divider">/</span></li>
  ...
  <li><a class="btn btn-link" href="<?php echo U('add');?>">新增栏目</a></li>
</ul>
...
<table class="table table-hover table-striped table-bordered">
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($vo["title_show"]); ?></td>
			<td><?php echo ($vo["type"]); ?></td>
			<td><?php echo ($vo["status"]); ?></td>
			<td>
			<a class="btn btn-mini" href="<?php echo U('edit?id='.$vo['cid']);?>">编辑</a>
			<a class="btn btn-mini ajax-get  confirm" href="<?php echo U('delete?id='.$vo['cid']);?>">删除</a>
			</td>
