...
<div class="row-fluid">
	<form class="form-horizontal" action="<?php echo U('add');?>" method="post">
		...
		<div class="control-group">
			<label class="control-label" for="article_id">文章标题</label>
			<div class="controls">
				<select id="article_id" name="article_id" class="span8">
                    <?php if(is_array($ArticleList)): $i = 0; $__LIST__ = $ArticleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
		</div>
        <div class="control-group">
            <label class="control-label" for="route_id">路线标题</label>
            <div class="controls">
                <select id="route_id" name="route_id" class="span8">
                    <?php if(is_array($RouteList)): $i = 0; $__LIST__ = $RouteList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
		<div class="control-group">
